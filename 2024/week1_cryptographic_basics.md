...
Lets see the workflow to have an overview of how a Circom circuit integrates to an App project:
```mermaid
sequenceDiagram
...
- **Proving/Verification keys:** The key files necessary for generating and verifying proofs. It is produced during the trusted setup phase of the circuit using Groth16, PLONK, etc. The files are **.zkey** for the proving key file and **.vkey.json** for verifing.
- **Contract:** This is the smart contract to be deployed to a blockchain or DLT to use it to verify the proofs generated from the App using the PoverModule, normally generated in Solidity (.sol) format.
